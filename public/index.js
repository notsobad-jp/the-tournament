  ...
  */}.toString().match(/(?:\/\*(?:[\s\S]*?)\*\/)/).pop().replace(/^\/\*/, "").replace(/\*\/$/, "").replace("{{tournamentId}}", id).replace("{{title}}", tournament.title);
  ...
  return minifyCss().then(function(css){
    ...
    if(tournament.profileImages) {
      header += '.profileImage { background-image: url("//'+ bucketName +'/'+ storage_root +'/'+ version +'/'+ id +'.png?t=20200924") }'
    }
...
var drawRoundOnly = function(orgId, roundOnly) {
  let id = orgId + "+round" + roundOnly;
